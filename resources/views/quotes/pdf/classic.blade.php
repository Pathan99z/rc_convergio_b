...
<body>
    ...
    <div class="totals">
        ...
        @if($quote->discount > 0)
        <p><strong>Discount Applied:</strong> -${{ number_format($quote->discount, 2) }}</p>
        @endif
        <p><strong>Tax ({{ $quote->items->first()->tax_rate ?? 0 }}%):</strong> ${{ number_format($quote->tax, 2) }}</p>
        <hr style="border: 1px solid #2c3e50; margin: 10px 0;">
        <p style="font-size: 18px; font-weight: bold;"><strong>TOTAL AMOUNT:</strong> ${{ number_format($quote->total, 2) }} {{ $quote->currency }}</p>
    </div>
